...
def printList(head):
    node = head
    while node != None:
        list_room_data.insert(END,node.data)
        node = node.link
...
def admin_show_roomuser_form():
    ru = Toplevel()
    ru.geometry('1010x600+250+100')
    ru.title('WELCOME TO | HA - HMS | ')
    ru.resizable(0,0)

    pht = PhotoImage(file='Room_Bookings_Data_Image.png')
    ...
    global list_room_data
    list_room_data = Listbox(ru, height=16, width=57,bg='light grey', font=('consolas', 15, 'bold'))
    list_room_data.place(x=320, y=180)
